...
まず html を内部データ構造へ parse する必要があります。
...
うまく動きました。ここでは plump element のベクトルが得られます。
...
これで page の link の list（正確にはベクトル）が得られました。次に、それらに到達できるかを確認して検証する async プログラムを書きます。
...
冒頭で見たように、`dex:get` は return code を含む多くの値を返します。ここでは `multiple-value-bind` で全てを受け取る代わりに、`nth-value` でこの値だけにアクセスします。また、エラーの場合に nil を返す `ignore-errors` を使います。`handler-case` を使って specific エラー type を扱うこともできます (dexador の documentation の例を参照してください)。
...
では、それらは何でしょうか。status code は見ましたが url は見ていません。すべての url のベクトルと、valid なもののベクトルがあります。単純にそれらを set として扱い、差分を計算します。これで悪いものがわかります。そのためにはベクトルを list に変換する必要があります。
