...
Common Lisp ではコンディションは *signaled* され、signaled されたコンディションに応じてコードを実行する過程を *handling* と呼びます。Java や C++ と違い、コンディションをハンドルすることは、ただちにスタックが巻き戻されることを意味しません。スタックを巻き戻すかどうか、どの状況でそうするかは、個々の handler 関数が決めます。
...
ありがたいことに `division-by-zero` の警告は得られますが、コードは実行され、2 つの値を返します。`nil` と、signal されたコンディションです。何を返すかは選べませんでした。

Slime では右クリックでコンディションを `inspect` できることを思い出してください。


<a id="handler-case--error-condition-"></a>
...
ここでも 2 つの値、0 と signal されたコンディションを返しました。
...
<a id="condition--restart--handler-bind"></a>

## コンディションと restart を完全に制御する: `handler-bind`

[handler-bind][handler-bind] は、コンディションが signal されたときに何が起きるかを完全に制御したい場合に使います。スタックを巻き戻しません。この点は次の節で示します。デバッガと restarts を、対話的にもプログラム的にも使えるようにします。
...
この挙動は、プログラムが simple コンディションを signal したときに特に便利です。simple コンディションはエラーではないため（下の「コンディション階層」を参照）、デバッガを起動しません。コンディション（アプリケーション内で何かが起きたという signal）に対して何か処理を行い、プログラムを続行できます。
...
<a id="condition-finallyunwind-protect"></a>
...
これで、コード内でコンディションを signal するときに、後で利用される情報を埋め込めます。
...
<a id="condition--signal--error-cerror-warn-signal"></a>

## コンディションを signal する: エラー, cerror, warn, signal

[エラー][error] は 2 通りに使えます。

- `(error "some text")`: [simple-error][simple-error] 型のコンディションを signal します。
- `(error 'my-error :message "We did this and that and it didn't work.")`: `message` スロットに値を与えたカスタムコンディションを作成し signal します。
...
その状況は、コードの通常動作中に情報を渡すことから、エラーのような重大な状況まで何でも構いません。たとえば、操作中の進捗を追跡するために使えます。`percent` スロットを持つコンディションを作り、進捗があるたびに signal し、上位コードがそれをハンドルしてユーザーに表示できます。詳しくは下の資料を参照してください。
...
コンディションを signal し得るコードがあるとします。ここでは `divide-with-restarts` がゼロ除算に関するエラーを signal する可能性があります。やりたいのは、上位レベルのコードがそれを自動的にハンドルし、適切な restart を呼ぶことです。
