...
`list*` pattern はオブジェクトの長さに関して柔軟です。
...
ただし、`list*` がオブジェクトを1つだけ受け取る場合、そのオブジェクトが list であるかどうかに関係なく、そのオブジェクトが返されることに注意してください。
...
`type` pattern はオブジェクトがその type であれば match します。`satisfies` は predicate がオブジェクトに対して true を返す場合に match します。lambda form も受け付けます。
...
`cl:getf` と同様に、`property` pattern に default 値を追加できます。`cl:getf` と異なり、その default が使われているかどうかを捕捉する flag をさらに追加できます。
