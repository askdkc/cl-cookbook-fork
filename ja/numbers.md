...
他の低水準プログラミング言語と同じく、Common Lisp は hexadecimal や 36 までの radix の literal 表記を提供します。例:
...
default の型は変更できますが、`single-float` 型を仮定しているパッケージを壊す可能性がある点に注意してください。
...
このエラーはハンドルできます。また、この behaviour を変更して `+infinity` を返すようにもできます。SBCL では次のようにします。
...
文字列と number の相互変換については [文字列セクション][strings] を参照してください。

## number の変換

ほとんどの numerical 関数は必要に応じて型を自動変換します。`coerce` 関数は、numeric type を含むオブジェクトをある型から別の型へ変換します。
...
むしろ、これは `single-float` が約 7 桁の精度を持つという意味です。一連の計算を行うとエラーが蓄積し、より大きなエラー margin が必要になることがあります。この場合は絶対差を比較できます。
...
多くの Common Lisp 関数はシーケンスを操作します。シーケンスは list またはベクトル（1 次元配列）です。[mapping][mapping] の section を参照してください。

多次元配列に対する operation は [this section][arrays] で説明しています。

number の「infinite」シーケンスを含む lazy シーケンスを定義し操作するライブラリもあります。例:
...
`format` 関数は `~@r` directive で number を roman numeral に変換できます。
...
[`random`][random] 関数は、その引数の型に応じて integer または floating point の random number を生成します。
...
random seed は [`*random-state*`][random-state] に保存され、その内部表現は処理系 dependent です。関数 [`make-random-state`][make-random-state] は、新しい random state を作成したり、既存の state をコピーしたりするために使えます。

同じ random number の集合を複数回使うには、`(make-random-state nil)` で現在の `*random-state*` のコピーを作成します。
...
これは loop 内で 10 個の random number を生成しますが、毎回シーケンスは同じです。なぜなら `*random-state*` special 変数が、`let` form の前でその state のコピーに動的束縛されるからです。

その他の資料:

* [random-state][random-state] パッケージは QuickLisp で利用でき、複数の portable random number generator を提供します。

## bit-wise operation

Common Lisp は bit-wise arithmetic operation を行うための関数も多く提供しています。よく使われるものを、対応する C/C++ 表現と一緒に以下に示します。
