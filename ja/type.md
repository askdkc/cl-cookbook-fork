...
Common Lisp には完全で柔軟な type system と、それに対応する type を調査・検査・操作する tools があります。custom types を作成し、変数や関数に type declarations を追加し、それによって compile-time warnings やエラーを得られます。
...
Common Lisp はマクロ [`check-type`][check-type] による run-time type checking をサポートしています。これは [`place`][place] と type specifier を引数として受け取り、place の内容が与えられた type でない場合に [`type-error`][type-error] を signal します。
...
しかし、[CLOS section][clos] で紹介した `:type` スロットと同様に、type declarations の効果は Lisp standard では undefined であり、処理系 specific です。そのため、Lisp compiler が compile-time type checking を行う保証はありません。
...
Note: `make-instance` 中にスロットの types を check する（compile time ではありません）data serialization/contract ライブラリである [sanity-clause][sanity-clause] も参照してください。
