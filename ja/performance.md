...
マクロ [`declaim`][declaim] はその可能性を提供します。file 内の top level form として使え、declarations は compile-time に作られます。
...
与えられた引数が指定された types を持つ場合にのみ効果があります。そうでなければエラーは signal されず、declaration は効果を持ちません。たとえば、次の declamation は、関数 `square` への引数が `fixnum` なら、その関数の値も `fixnum` になることを述べています。
...
パッケージ [`inlined-generic-function`][inlined-generic-function] は、generic 関数を static dispatch に変換し、dispatch cost を compile-time に移す関数を提供します。generic 関数を `inlined-generic-function` として定義するだけで済みます。
