...
array を操作するために [Quicklisp](https://www.quicklisp.org/beta/) で利用できるライブラリには、次のものがあります。
...
- [array-operations](https://github.com/Lisp-Stat/array-operations) は lisp-stat project 由来で、
  ...
  `combine`, `reshape` 関数を定義します。また、element-wise
...
- [lla](https://github.com/tpapp/lla) は linear algebra 用ライブラリで、BLAS と LAPACK
  ...
  直感的な関数 name を使い、native array だけでなく
  CLOS オブジェクトに対しても動作できます。

この page では built-in の多次元 array でできることを扱いますが、制限もあります。特に次の点です。

* 外部 language array との interoperability。たとえば BLAS、LAPACK、GSL のようなライブラリを呼び出す場合です。
* arithmetic operator やその他の mathematical operator を、array を扱えるように拡張すること。たとえば `a` や `b` が array のときに `(+ a b)` が動くようにする場合です。

これら 2 つの問題は、native array を special case として持つ extended array クラスを CLOS で定義することで解決できます。この approach を取る [quicklisp](https://www.quicklisp.org/beta/) 経由で利用可能なライブラリには、次があります。
...
dependency (BLAS、LAPACK、Expokit) の install 手順は [github web pages](https://github.com/rigetticomputing/magicl) にあります。low-level routine は外部関数を wrap しているため、たとえば `magicl.lapack-cffi::%zgetrf` のような Fortran name を持ちます。これらの関数の一部には higher-level インターフェイスも存在します。[source directory](https://github.com/rigetti/magicl/blob/master/src/high-level/) と [documentation](https://github.com/quil-lang/magicl/blob/master/doc/high-level.md) を参照してください。
...
関数 [CLHS: make-array](http://clhs.lisp.se/Body/f_mk_ar.htm) は、単一の値で満たされた array を作成できます。
...
より複雑な array 値は、まず array を作り、その後 element を反復して値を埋めることで生成できます (element access については下の section を参照してください)。

[array-operations](https://github.com/tpapp/array-operations) ライブラリは、この iteration を wrap して array を作成する便利な関数 `generate` を提供します。
...
`array-operations` の nickname は `aops` であることに注意してください。`generate` 関数は key `:subscripts` を渡すことで array subscript も反復できます。さらに多くの例は [Array Operations manual on generate](https://lisp-stat.dev/docs/manuals/array-operations/#generate) を参照してください。

### 乱数

uniform distribution から引いた random number を含む 3x3 array を作るには、`generate` を使って CL の [random](http://clhs.lisp.se/Body/f_random.htm) 関数を呼び出せます。
...
[alexandria](https://common-lisp.net/project/alexandria/) パッケージを使った、mean が 0、standard deviation が 1 の Gaussian (normal) random number の array です。
...
これは特に効率的ではないことに注意してください。各 element ごとに関数 call が必要であり、`gaussian-random` は 2 つの random number を返しますが、そのうち 1 つしか使われません。

より効率的な実装や、より広い範囲の probability distribution には、Quicklisp で利用できるパッケージがあります。一覧は [CLiki](https://www.cliki.net/statistics) を参照してください。

## element への access

array の個々の element に access するには、[aref](http://clhs.lisp.se/Body/f_aref.htm) と [row-major-aref](http://clhs.lisp.se/Body/f_row_ma.htm#row-major-aref) 関数があります。

[aref](http://clhs.lisp.se/Body/f_aref.htm) 関数は、array の dimension 数と同じ数の index 引数を取ります。indexing は 0 から始まり、C と同じ row-major ですが、Fortran とは異なります。
...
別の matrix-multiply の実装については、下の linear algebra section を参照してください。

## Element-wise operations

同じ size の 2 つの number array を掛け合わせるには、[array-operations](https://github.com/Lisp-Stat/array-operations) ライブラリの `each` に関数を渡します。
...
効率を高めるために `aops:each*` 関数があります。これは result array を specialize するため、最初の引数に type を取ります。
...
`each` は destructive ではなく、新しい array を作ることに注意してください。`each` へのすべての引数は同じ size の array でなければならないため、`(aops:each #'+ 42 *a*)` は valid ではありません。

<a id="expression--vectorize"></a>

### 式の vectorize

上の `each` 関数に対する代替 approach は、array のすべての element を反復するマクロを使うことです。
...
式の body (`vectorize` 式の 2 番目の form) の中では、シンボル `*a*` は single element に bind されます。つまり built-in mathematical 関数を使えるということです。
...
fast matrix manipulation のために、いくつかのパッケージが BLAS の wrapper を提供しています。
...
これは `a * x + y` を計算します。ここで `a` は constant、`x` と `y` は array です。`lla:axpy!` 関数は destructive で、最後の引数 (`y`) を変更します。
...
[reduce](http://clhs.lisp.se/Body/f_reduce.htm) 関数は vector (1D array) を含む sequence に対して動作しますが、多次元 array には動作しません。これを回避するには、多次元 array を displaced して 1D vector を作成できます。displaced array は original array と storage を共有するため、data copy を必要としない fast operation です。
...
`array-operations` パッケージには `flatten` があり、displaced array、つまり data を copy しない array を返します。
...
ときには、より複雑な reduction が必要です。たとえば 2 つの array の maximum absolute difference を求める場合です。上のメソッドを使うと次のようにできます。
...
これは intermediate result を保持する array の allocation を伴い、大きな array では非効率になり得ます。上で定義した `vectorize` と同様に、allocate しないマクロを次のように定義できます。
...
このマクロを使うと、任意の shape の array A における maximum 値は次のように求められます。
...
いくつかのパッケージが BLAS と LAPACK ライブラリへの binding を提供しています。たとえば次です。
...
[lla](https://github.com/tpapp/lla) 関数 `mm` は vector-vector、matrix-vector、matrix-matrix multiplication を行います。
...
[array-operations](https://github.com/Lisp-Stat/array-operations) パッケージには generalised [outer product](https://en.wikipedia.org/wiki/Outer_product) 関数が含まれます。
...
これは新しい 2D array `A[i j] = B[i] * C[j]` を作成しています。この `outer` 関数は任意個の input と、multiple dimension を持つ input を取れます。
...
`svd` 関数は、与えられた matrix の [singular 値 decomposition](https://en.wikipedia.org/wiki/Singular-value_decomposition) を計算し、返される 3 つの matrix 用のスロットを持つオブジェクトを返します。
...
diagonal matrix (singular 値) と vector は関数で access できます。
...
[Matlisp](https://github.com/bharath1097/matlisp/) scientific computation ライブラリは、BLAS と LAPACK 関数の wrapper を含む、array に対する high performance operation を提供します。quicklisp で load できます。

~~~lisp
* (ql:quickload :matlisp)
~~~

`matlisp` の nickname は `m` です。各シンボルの前に `matlisp:` や `m:` と入力するのを避けるには、matlisp を use する自分用のパッケージを定義できます ([PCL section on パッケージ](http://www.gigamonkeys.com/book/programming-in-the-large-packages-and-symbols.html) を参照)。
...
1D array を生成するには `range` と `linspace` 関数があります。
...
dimension を指定して `tensor` クラスの instance を作成することもできます。`tensor` オブジェクトの internal storage は、スロット `store` 内の 1D array (`simple-vector`) です。
...
array store はスロットを使って access できます。
...
`ref` 関数は standard CL array に対する `aref` に相当し、setf-able でもあります。
...
`matlisp` が load されたときに load される `matlisp-user` パッケージには、tensor に対して element-wise に操作する関数が含まれます。
