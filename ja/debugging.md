...
[*break-on-signals*](https://cl-community-spec.github.io/pages/002abreak_002don_002dsignals_002a.html) は、エラー（または任意のコンディション）が発生したことは分かるがデバッガが出ず、エラー（または任意のコンディション）が signal される*直前*にデバッガを強制的に開きたいとき、特に役立ちます。
...
デバッガから、スタックトレースを調べ、コンディションを signal した行へ移動し、修正して実行を再開できます。

signal されたコンディションがエラーの場合、break を処理した後に 2 回目のデバッガには入りません。
...
`watch` は、監視中のオブジェクトへスレッドが書き込もうとしたときにコンディションを signal します。GUI 内で監視対象オブジェクトを表示する機能と組み合わせることもできます。ある種のバグ（誰かがこの値を変更しているが、誰か分からない）には、非常に役立つことがあります。
...
最後に、独立した関数の自動テストこそ探しているものかもしれません！ [testing](testing.html) セクションと [test frameworks and ライブラリ](https://github.com/CodyReichert/awesome-cl#unit-testing) の一覧を参照してください。


<a id="remote-debugging"></a>
