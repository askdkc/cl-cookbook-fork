...
pathname (`#p`)、trailing slash、`:validate` キーとともに `uiop:delete-directory-tree` を使います。
...
`mysystem` の中で作業しているとします。これは ASDF system declaration を持ち、その system は Lisp image にロードされています。この ASDF file は filesystem のどこかにあり、`src/web/` への path が欲しいとします。次のようにします。
...
*   `<_if-does-not-exist_>` は、開きたい file が存在しない場合にどうするかを指定します。エラーを通知する `:error`、空の file を作成する `:create`、ストリーム変数を `nil` に束縛する `nil` のいずれかです。default は簡単に言えば、指定した他の option に応じて正しいことをする、というものです。詳細は CLHS を参照してください。
...
file の内容に文字列形式でアクセスしたり、line の list を取得したりする必要はかなりよくあります。

uiop は ASDF に含まれており (追加で install するライブラリやロードする system はありません)、次の関数を持ちます。
...
さらに、常に文字型の element を使う代わりに、read/write されるデータの format を自由に変更できます。たとえば octet としてデータを読むには、`with-output-to-string`、`with-open-file`、`make-array` 関数の `:element-type` type 引数を `'(unsigned-byte 8)` に設定できます。
...
この contrib は POSIX system では default でロードされます。
...
上の関数に加えて、directory を *lazy* に traverse する解決策にも触れておきます。これらは file の list 全体をロードしてから返すわけではありません。
