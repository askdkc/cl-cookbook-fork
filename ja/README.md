...
生成物は `ja/common-lisp-cookbook-ja.epub` と `ja/common-lisp-cookbook-ja.pdf` です。`make` の既定ターゲットは `ja/` 以下の Markdown、`ja/metadata.txt`、`ja/epub.css`、`ja/orly-cover.png` を使って生成物を作ります。コードブロックの syntax highlight には Shiki の light theme を使います。
...
epub には、ある程度新しいバージョンの [Calibre](https://calibre-ebook.com/) が必要です。簡単なバイナリ installation が提供されています。
...
このプロジェクトは Common Lisp クックブックをこの10年にふさわしいものにします。SourceForge にあった元の Common Lisp クックブックの開発は 2007 年に停止しました。その間、Common Lisp の世界では多くのことが起こりました。tool や処理系は改善され、使われなくなったものもあります。特に、Common Lisp user は今では [Quicklisp][ql] ライブラリ manager の恩恵を受けられます。

主な目標は、content の更新と拡張に加えて、クックブックをより現代的で、よりアクセスしやすいものにすることです。
...
必要な software dependency がすべて利用可能になる development 環境に入るには、project root directory で `guix shell` を実行します。提供されている Guix [manifest file](manifest.scm) は、Guix がそうするための [authorization](https://guix.gnu.org/manual/devel/en/html_node/Invoking-guix-shell.html) を与えた後、自動的に読み込まれます。

## 支援

クックブックを継続的に改善している個人を支援できます。Github Sponsors icon を参照してください。彼らに感謝します。
